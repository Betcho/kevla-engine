...
export default function SplashScreen({ onComplete }: SplashProps) {
  ...
  useEffect(() => {
    const steps = [
      ...
      { p: 80, s: 'Applying dark theme...' },
      { p: 85, s: 'Loading project configuration...' },
      { p: 90, s: 'Initializing Temporal Engine...' },
      { p: 95, s: 'Creating default scene...' },
      { p: 100, s: 'Ready.' },
    ];
    ...
    <div className={`splash-overlay ${fadeOut ? 'fade-out' : ''}`}>
      <div className="splash-card">
        ...
        <div className="splash-logo-area">
          ...
          <p className="splash-subtitle">3D Game Engine & Editor</p>
        </div>
        ...
        <div className="splash-footer">
          <span>© 2024 KEVLA Engine</span>
          <span>KevlaEditor.exe</span>
        </div>
      </div>
    </div>
  );
